...
def update_paths(paths):
    ...
    paths['partial_aligned_prediction'] = paths['output'] + 'partial_aligned_prediction.pdb'
...
def score(paths):
    try:
        ...
        f = os.popen(paths['phenix_chain_comparison_path'] + ' ' + paths['partial_aligned_prediction'] + ' ' + paths['partial_ground_truth'] + ' | grep \'Unique_target\'')
        line = ''
...
def align(paths):
    try:
        f = os.popen(paths['tmalign_path'] + ' ' + paths['partial_prediction'] + ' ' + paths['partial_ground_truth'] + ' -o ' + paths['output'] + 'TM.sup')
        time.sleep(1)
        ap_file = open(paths['partial_aligned_prediction'], 'w')
